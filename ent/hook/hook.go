...
// Mutate calls f(ctx, m).
func (f RegisteredContractFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RegisteredContractMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RegisteredContractMutation", m)
}

// The RegisteredProviderFunc type is an adapter to allow the use of ordinary
// function as RegisteredProvider mutator.
type RegisteredProviderFunc func(context.Context, *ent.RegisteredProviderMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f RegisteredProviderFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RegisteredProviderMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RegisteredProviderMutation", m)
}
