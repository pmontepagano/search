...
var (
	...
	usePythonBisimulation = flag.Bool("use_python_bisimulation", false, "Use the Python Bisimulation library to check for bisimulation")
)

func main() {
	flag.Parse()
	b := broker.NewBrokerServer(*databaseFile)
	if *usePythonBisimulation {
		b.SetCompatFunc(broker.BisimilarityAlgorithm)
	}
